...
function roundToPowerOfTen(num, pow) {
  ...
  const temp = num % powOfTen;

  if (temp < powOfTen / 2) return num - temp;
  return num + (10 ** pow - temp);
}
...
function getSumOfDigits(num) {
  ...
  for (let i = 0; i < temp.length; i += 1) {
    result += Number.parseInt(temp.charAt(i), 10);
  }
  return result;
}
...
function getCountOfOddNumbers(number) {
  let count = 0;
  for (let i = 0; i < Math.abs(number); i += 2) count += 1;
  return count;
}
